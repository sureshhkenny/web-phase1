...
class Blocks extends React.Component {
    ...
    attachVM() {
        ...
        this.workspace.addChangeListener(this.onBlockChange)
    ...
    detachVM() {
        ...
        this.workspace.removeChangeListener(this.onBlockChange)
    }
    ...
    onTargetsUpdate() {
        ///on click of the block updateing
    ...
    onBlockChange() {
        JavaScriptGenerator.setVariableMap = this.workspace.getVariableMap()
        JavaScriptGenerator.init(this.workspace)
        JavaScriptGenerator['data_setvariableto'] = function (block) {
            var id = (block.getFieldValue('VARIABLE'));
            var variable, code;
            var value = JavaScriptGenerator.setVariableMap.variableMap_
            var data = Object.values(value)[0].map(item => {
                if (item.id_ == id) {
                    variable = item.name
                }
            })
            var valuetocode = JavaScriptGenerator.valueToCode(block, 'VALUE',
                JavaScriptGenerator.ORDER_NONE) || "0"
            code = JavaScriptGenerator.INDENT + `var ${variable} = ${valuetocode}`
            return code
        }
        JavaScriptGenerator['data_changevariableby'] = function (block) {
            ...
            code = JavaScriptGenerator.INDENT + `${variable} = ${variable}${valuetocode}`
            return code
        };
        ...
        JavaScriptGenerator['data_variable'] = function (block) {
            ...
            JavaScriptGenerator.scrub_(block, code, true)
            code = `${variable}`
        ...
        const blocksupdate = this.props.vm.editingTarget;
    ...
    onWorkspaceUpdate(data) {
        ...
        if (toolboxXML) {
            ...
            const workspacetodom = this.ScratchBlocks.Xml.workspaceToDom(this.workspace);
