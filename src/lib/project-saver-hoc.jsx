...
const ProjectSaverHOC = function (WrappedComponent) {
    class ProjectSaverComponent extends React.Component {
        ...
        componentDidUpdate(prevProps) {
            ...
            if (this.props.loadingState === LoadingState.SHOWING_WITHOUT_ID) {
                this.props.onCreateProject();
            }
        }
        ...
        updateProjectToStorage() {
            ...
            return this.storeProject(this.props.reduxProjectId,this.props.reduxProjectTitle)
                .then(() => {
        ...
        createNewProjectToStorage() {
            return this.storeProject(null)
                .then(response => {
                    this.props.onCreatedProject(response.data.game.id, this.props.loadingState);
                })
                .catch(err => {
         ...
         */
        storeProject(projectId, projectTitle, requestParams) {
            ...
            const vmAsset = this.props.vm.assets;
            for (var i = 0; i < vmAsset.length; i++) {
                if (vmAsset[i].clean == false) {
                    const assetInfo = vmAsset[i];
                    const buffer = vmAsset[i].data
                    const dataUri = assetInfo.encodeDataURI();
                    const uri = dataUri.split(',')[1]
                    const formData = {
                        assetId: assetInfo.assetId,
                        data: uri,
                        assetType: assetInfo.assetType.contentType,
                        dataFormat: assetInfo.dataFormat
                    }
                    const headers = {
                        headers: {
                            'Content-Type': 'image/png',
                            'Authorization': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2MDRkOWMwMS01YjhhLTQ2NzYtYWJhYS01NDBlOTJiZjBkODIiLCJpYXQiOjE2NjczNjY0NjZ9.DqXtFgADeveZcVrqas0Zuasd9Qm8CoLSJSf7-hNj_Lk'
                        },

                    };
                    const data = vmAsset[i].data
                    axios.post("/master/asset/upload", formData, {
                        headers: {
            ...
            return Promise.all(this.props.vm.assets
                ...
                .then(() => {
                    ...
                    return this.props.onUpdateProjectData(projectId, savedVMState,projectTitle)
                })
                // .then(response => {
                //     this.props.onSetProjectUnchanged();
